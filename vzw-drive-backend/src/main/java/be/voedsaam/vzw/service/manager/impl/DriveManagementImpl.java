...
public class DriveManagementImpl implements DriveManagement {
	...
	public Collection<DriveDTO> getDrivesByDriver(UserDTO user) {
		//return driveMapper.mapToDTO(driveRepository.getAllWithDriver(userMapper.mapToObj(user)));
		
		return driveMapper.mapToDTO(driveRepository.getAll()
				.stream()
	...
	public Collection<DriveDTO> getDrivesByAttendee(UserDTO user) {
		//return driveMapper.mapToDTO(driveRepository.getAllWithAttendee(userMapper.mapToObj(user)));
		
		return driveMapper.mapToDTO(driveRepository.getAll()
				.stream()
				.filter(d -> d.getAttendee()!=null)
	...
	public Collection<DriveDTO> getDrivesByDepotHelp(UserDTO user) {
		//return driveMapper.mapToDTO(driveRepository.getAllWithDepotHelp(userMapper.mapToObj(user)));
		return driveMapper.mapToDTO(driveRepository.getAll()
				.stream()
				.filter(d -> d.getDepotHelp()!=null)
	...
	public Collection<DriveDTO> getDrivesByDestination(DestinationDTO destination) {
		//return driveMapper.mapToDTO(driveRepository.getAllWithDestination(destinationMapper.mapToObj(destination)));
		return driveMapper.mapToDTO(driveRepository.getAll()
				.stream()
				.filter(d -> d.getDestinations()!=null)
				.filter(d ->d.getDestinations().contains(destinationMapper.mapToObj(destination))).collect(Collectors.toList()));
	}
	...
	@Override
	public DriveDTO addDestination(DriveDTO drive1, DestinationDTO destinationDTO) {
	...
	public Collection<DestinationDTO> getDestinationsByDrive(DriveDTO driveDTO) {
		...
		List<Destination> results = new ArrayList<Destination>(driveRepository.getByID(driveDTO.getId()).getDestinations());
